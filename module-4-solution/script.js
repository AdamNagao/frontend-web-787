...
function getGreetingsArrays(previousValue, currentValue){
  var firstLetter = currentValue.charAt(0).toLowerCase();
  var newArray = {hello: [], bye: []};

  newArray.hello = previousValue.hello.slice();
  newArray.bye = previousValue.bye.slice();

  if (firstLetter === 'j') {
    newArray.bye.push(byeSpeaker.speakSimple(currentValue));
  } else {
    newArray.hello.push(helloSpeaker.speakSimple(currentValue));
  }
...
var mapNames = names.map(makeGreetings);

for (var i = 0; i < mapNames.length; i++) {
  console.log(mapNames[i]);
}

var reduceNames = names.reduce(getGreetingsArrays, {hello: [], bye: []})
